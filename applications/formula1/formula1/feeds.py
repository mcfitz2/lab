...
class Processor:
    ...
    def process_entry(self, entry):
        if 'formula.1' in entry['title'].lower() or 'formula 1' in entry['title'].lower():
...
class Release(BaseModel):
    ...
    @classmethod
    def transformName(cls, raw: str) -> str:
        common_mappings = {
            "USA": "United States",
            "Sao Paulo": "Brazil",
            "Abu-Dhabi": "Abu Dhabi",
        ...
        fixed = ' '.join(re.findall('[A-Z][^A-Z]*', raw.replace('GP', '').replace('.', ' '))).replace('  ',
                                                                                                      ' ').replace('-',
                                                                                                                   ' ')
        try:
            without_GP = raw.split('GP')[0]
            if without_GP in demonyms.demonyms.keys():
        ...
        try:
            ...
            pass
        if fixed in demonyms.demonyms.keys():
    ...
    @classmethod
    def transformRound(cls, raw: str) -> int:
        return int(raw)

    @field_validator("episode", mode="before")
    @classmethod
    def transformEpisode(cls, raw: str) -> int:
        return int(raw)

    @field_validator("year", mode="before")
    @classmethod
    def transformYear(cls, raw: str) -> int:
        return int(raw)

    @field_validator("quality", mode="before")
    @classmethod
    def transformQuality(cls, raw: str) -> str:
        if raw.upper() == '4K':
            return '2160P'
        else:
            return raw.upper()
    ...
    @classmethod
    def transformEvent(cls, raw: str) -> str:
        pt1 = ['Pre-Season.Test.Day1', 'Pre.Season.Testing.Day.1', 'Pre.Season.Testing.Day.1']
        pt2 = ['Pre-Season.Test.Day2', 'Pre.Season.Testing.Day.2', 'Pre.Season.Testing.Day.2']
        pt3 = ['Pre-Season.Test.Day3', 'Pre.Season.Testing.Day.3', 'Pre.Season.Testing.Day.3']
        if raw:
            if raw in pt1:
...
def get_episode_for_round(season, release):
    ...
    if release.event == Event.TESTING3:
        return season[2]
    if release.episode:
        options = [episode for episode in season if int(release.episode) == int(episode['number'])]
        if len(options) > 0:
            return options[0]
    if release.round:
        by_round = events_by_round(season)
        if release.year == 2023 and release.round > 5:
            round = by_round[release.round - 1]
        else:
            round = by_round[release.round]

        for episode in round:
            if release.event != Event.SPRINT and release.event in episode['name']:
                return episode
            elif release.event == Event.SPRINT and release.event in episode['name'] and Event.SHOOTOUT not in episode['name']:
                return episode
    return None
    ...
    def get_releases(self, season, episode, allow_weekend=False):
        with self.lock:
            ...
            if self.last_check and seconds < 600:
                ...
                return [(release, entry, ep)
                        for release, entry, ep in self.releases
                        if (season and episode and int(ep['number']) == int(episode)
            ...
            tvdb = tvdb_v4_official.TVDB('316d9ad1-71cf-47a0-9ad1-689bb598522c', pin='AVUYMSV3')
            series = tvdb.get_series_extended(387219)
            seasons = {
                season['number']: sorted([episode for episode in tvdb.get_season_extended(season['id'])['episodes']],
                                         key=lambda x: x['number']) for season in series['seasons'] if
                season['number'] > 2018}
            ...
            for processor in processors:
                for release, entry in processor.process():
                    if release:
                        if release.event in [Event.TESTING1, Event.TESTING2, Event.TESTING3, Event.RACE, Event.SPRINT,
                                             Event.SHOOTOUT, Event.QUALIFYING, Event.FP1, Event.FP2,
                                             Event.FP3] or allow_weekend:
                            ep = get_episode_for_round(seasons[release.year], release)
                            if ep is not None:
                                if (season and episode and int(ep['number']) == int(episode) and int(
                                        ep['seasonNumber']) == int(season)) or not season and not episode:
