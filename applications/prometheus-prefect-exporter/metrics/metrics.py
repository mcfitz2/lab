...
class PrefectMetrics(object):
    ...
    def collect(self):
        ...
        prefect_deployment_up = GaugeMetricFamily(
            "prefect_deployment_up",
            "Last flow run status for deployment",
            labels=[
                "deployment_id",
                "deployment_name"
            ],
        ...
        for flow_run in all_flow_runs:
            ...
            if flow_run.get("state_name") == 'Scheduled' or deployment_name == 'null':
                continue

            ls = last_states.get(deployment_name)
            if ls:
                fmt = "%Y-%m-%dT%H:%M:%S.%f%z"
                if ls.get('end_time') and flow_run.get('end_time') and datetime.datetime.strptime(ls.get('end_time'), fmt) < datetime.datetime.strptime(flow_run.get('end_time'), fmt):
                    last_states[deployment_name] = flow_run
            else:
                last_states[deployment_name] = flow_run
        for deployment_name, flow_run in last_states.items():
            # set state
            state = 0 if flow_run.get("state_name") == "Failed" else 1
            prefect_deployment_up.add_metric(
                [
                    str(flow_run.get("deployment_id", "null")),
                    str(deployment_name),
                ],
